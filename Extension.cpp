    ...
    : rdPtr(_rdPtr), rhPtr(_rdPtr->rHo.hoAdRunHeader), Runtime(_rdPtr)
{
    ...
    */
	...
	LinkAction(0, EnumerateAllDevices);
	...
	LinkExpression(0, DeviceCount);
	LinkExpression(1, InputBuffer);
	LinkExpression(2, ManufacturerName);
	LinkExpression(3, ProductName);
	LinkExpression(4, SerialNumber);
	LinkExpression(5, ProductId);
	LinkExpression(6, VendorId);
	LinkExpression(7, LastError);
	LinkExpression(8, OutputBufferLength);
    ...
    */
	...
	curId = 0;
	readBytes = 0;
	//_mbscpy_s(readBuffer, 1, static_cast<unsigned char>(0));
